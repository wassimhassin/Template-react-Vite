...
const Reclamations = () => {
  ...
  // Filtrer uniquement les réclamations non fermées
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col no-scrollbar">
      ...
      <div className="sticky top-0 z-30 bg-white border border-gray-200 rounded-b-none rounded-t-xl shadow-lg mb-0 mx-2 sm:mx-6 mt-2 sm:mt-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Gestion et suivi des réclamations ouvertes et en cours</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <button className="group relative flex items-center space-x-2 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-orange-500 hover:to-yellow-500 text-white font-medium rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 overflow-hidden">
        ...
        <div className="p-4 sm:p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center space-x-3 p-3 bg-orange-50 rounded-lg">
              ...
              <div>
                <p className="text-sm text-gray-600">Total Réclamations</p>
      ...
      {/* Tableau des réclamations + Pagination */}
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className={`font-medium ${isHighlightedTicket(ticket) ? 'text-red-900' : 'text-gray-900'}`}>{ticket.number}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{ticket.msisdn}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          value={ticket.statut}
                          onChange={(e) => handleStatusChange(ticket.id, e.target.value)}
                          className={`px-2 py-1 text-xs font-semibold rounded-full border-0 ${getStatusColor(ticket.statut)} focus:ring-2 focus:ring-orange-400`}
                        >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800`}>{ticket.type}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{ticket.sousType}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getPriorityColor(ticket.priorite)}`}>{ticket.priorite}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{ticket.dateCreation ? new Date(ticket.dateCreation).toLocaleString('fr-FR', { dateStyle: 'short', timeStyle: 'short' }) : '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{ticket.agent}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          <button className="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50" title="Voir détails">
                            <Eye className="w-4 h-4" />
                          </button>
                          <button className="text-green-600 hover:text-green-900 p-1 rounded hover:bg-green-50" title="Modifier">
                            <Edit3 className="w-4 h-4" />
                          </button>
                          <button className="text-purple-600 hover:text-purple-900 p-1 rounded hover:bg-purple-50" title="Assigner">
